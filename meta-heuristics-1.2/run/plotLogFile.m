...
%                 pl(1)= plot( time(i), best(i), 'Color', cmap( proc(i) + 1,:), 'LineWidth', 4.0);
...
%                 pl(4) = plot( time(i), worst(i), 'b:');
    ...
    for j = 0 : numProc() - 1  
        for i = 0 : numTask() - 1  
            if j == proc(i)
                ...
                elseif strcmp(heurictis(i), 'GA') || strcmp(heurictis(i), 'MA')
                    graph = plot( time(i), best(i), 'Color', cmap( proc(i) + 1,:), 'LineWidth', 2.0);  
                end
                if isempty(graph) == 0
                    dum = [dum ; graph];
                    if isempty(text)
    ...
    for i = 0 : numTask() - 1  
        if strcmp(heurictis(i), 'GA') == 1  
            graph = plot( time(i), best(i), 'sc', 'MarkerSize', 3.0, 'MarkerEdgeColor', 'c' ,'MarkerFaceColor', 'c' );
%             graph = errorbar( time(i), average(i), stdd(i), 'sb', 'MarkerSize', 8.0); 
    ...
    for i = 0 : numTask() - 1  
        if strcmp(heurictis(i), 'MA') == 1  
            graph = plot( time(i), best(i), 'sm', 'MarkerSize', 3.0, 'MarkerEdgeColor', 'm' ,'MarkerFaceColor', 'm' );      
